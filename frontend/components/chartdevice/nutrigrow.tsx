...
const ChartNutrigrow: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      ...
      const filteredDevices = devicesData.filter((device: Device) => device.type === 1);
  ...
  useEffect(() => {
    if (selectedApiKey) {
      const fetchData = async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/go/chartnutrigrow?apikey=${selectedApiKey}&timeFilter=${timeFilter}`);
  ...
  useEffect(() => {
    Object.keys(data).forEach((monthStr) => {
      ...
      if (chartRefs.current[month]) {
        ...
        const ph = data[month]?.map((d) => d.ph) ?? [];
        const ec = data[month]?.map((d) => d.ec) ?? [];
        const doValue = data[month]?.map((d) => d.do) ?? [];
