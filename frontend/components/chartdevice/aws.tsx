...
const ChartAWS: React.FC = () => {
  ...
  const [firstMonthExpanded, setFirstMonthExpanded] = useState<boolean>(true);
    ...
    Object.keys(data).forEach((monthStr) => {
      ...
      if (chartRefs.current[month]) {
        ...
        const dates = data[month]?.map((d) => {
          if (timeFilter === '24H') {
            return d.created_at.split('T')[1].split('Z')[0];
          } else {
        ...
        const airpressure = data[month]?.map((d) => d.airpressure) ?? [];
        const windspeed = data[month]?.map((d) => d.windspeed) ?? [];
        const winddirection = data[month]?.map((d) => d.winddirection) ?? [];
        ...
        const option: EChartsOption = {
          tooltip: {
            ...
            formatter: (params: any) => {
                ...
                const tooltipDate = datesTooltip[dataIndex];
          ...
          legend: {
            data: ['Temperature', 'Humidity', 'Air Pressure', 'Wind Speed', 'Wind Direction'],
            left: 50,
            top:30,
          },
          grid: {
        ...
        chartRefs.current[month]!.style.borderRadius = '8px';
  ...
  useEffect(() => {
    if (timeFilter === '1Y') {
      setFirstMonthExpanded(true);
    } else {
      setFirstMonthExpanded(false);
    }
  }, [timeFilter]);
      ...
      {timeFilter !== '1Y' ? (
        ...
        <Collapse.Group>
          {Object.keys(data).map((monthStr) => {
            const month = parseInt(monthStr, 10);
            return (
              <Collapse key={month} title={months[month - 1]} expanded={month === 1 && firstMonthExpanded}>
                <div ref={ref => chartRefs.current[month] = ref} style={{ width: '100%', height: '500px' }} />
              </Collapse>
            );
          })}
        </Collapse.Group>
